            ...
            {Array.from({ length: daysInMonth }).map((_, idx) => {
              ...
              return (
                <button
                  ...
                  className={`h-8 w-8 mx-auto rounded-xl flex items-center justify-center transition-all duration-150 cursor-pointer ${
                    isSelected
  ...
  return (
    <div className="w-full space-y-6 min-h-screen text-slate-800 dark:text-slate-100 p-2 sm:p-4 lg:p-6 font-sans antialiased select-none relative transition-colors duration-300">
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-lg overflow-hidden transition-colors duration-300">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-5 pt-7">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-indigo-700 to-violet-700 dark:from-indigo-400 dark:to-violet-300">Mi Perfil</h1>
              <p className="text-xs text-slate-400 dark:text-slate-400 font-bold uppercase tracking-widest mt-0.5">Configuración de cuenta y datos institucionales</p>
            </div>
          </div>
          <div className="flex items-center gap-2 bg-violet-50 dark:bg-violet-950/60 px-4 py-2 rounded-xl border border-violet-100 dark:border-violet-900/60 text-[11px] font-black uppercase tracking-wider text-violet-600 dark:text-violet-300 shadow-xs">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start w-full">
        ...
        <div className="xl:col-span-4 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/60 dark:border-slate-800 shadow-md hover:shadow-xl transition-all duration-300 p-6 text-center space-y-6 relative overflow-hidden group">
          ...
          <div className="relative pt-8 z-10">
            <div className={`w-32 h-32 ${avatarUrl ? '' : selectedBg} mx-auto rounded-3xl flex items-center justify-center text-4xl font-black uppercase shadow-xl border-4 border-white dark:border-slate-800 overflow-hidden transition-all duration-300 transform group-hover:scale-105`}>
              {avatarUrl ? (
            ...
            <button 
              ...
              className="absolute bottom-1 right-1/2 translate-x-16 bg-slate-900 dark:bg-slate-800 text-white p-2.5 rounded-xl shadow-md border border-white/20 dark:border-slate-700 hover:bg-slate-800 dark:hover:bg-slate-700 transition-all duration-200 cursor-pointer hover:scale-110 active:scale-95"
          ...
          <div className="space-y-1">
            <h3 className="text-xl font-black text-slate-900 dark:text-slate-100 tracking-tight capitalize truncate">{name || user?.name}</h3>
            <p className="text-xs font-bold text-slate-400 dark:text-slate-400 font-mono truncate">{email || user?.email}</p>
          </div>

          <div className="grid grid-cols-2 gap-3 pt-2">
            <div className="bg-indigo-50/70 dark:bg-indigo-950/50 border border-indigo-100 dark:border-indigo-900/50 p-3.5 rounded-2xl flex flex-col items-center justify-center transition hover:bg-indigo-100/70 dark:hover:bg-indigo-900/60 hover:shadow-md">
              <Shield className="text-indigo-600 dark:text-indigo-400 mb-1.5" size={20} />
              <span className="text-[10px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-wider">Rol Sistema</span>
              <span className="text-sm font-black text-indigo-700 dark:text-indigo-300 mt-1 uppercase">{user?.role}</span>
            </div>
            <div className="bg-fuchsia-50/70 dark:bg-fuchsia-950/50 border border-fuchsia-100 dark:border-fuchsia-900/50 p-3.5 rounded-2xl flex flex-col items-center justify-center transition hover:bg-fuchsia-100/70 dark:hover:bg-fuchsia-900/60 hover:shadow-md">
              <Heart className="text-fuchsia-500 dark:text-fuchsia-400 fill-fuchsia-500/10 mb-1.5" size={20} />
              <span className="text-[10px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-wider">Asignación</span>
              <span className="text-sm font-black text-fuchsia-700 dark:text-fuchsia-300 mt-1 uppercase truncate max-w-full">{user?.groupRole || 'MIEMBRO'}</span>
            </div>
          </div>

          <div className="bg-gradient-to-r from-violet-50 to-fuchsia-50 dark:from-violet-950/40 dark:to-fuchsia-950/40 border border-violet-100 dark:border-violet-900/50 rounded-2xl p-4 flex items-center gap-3 text-left shadow-sm">
            <div className="p-2.5 bg-white dark:bg-slate-800 rounded-xl text-violet-600 dark:text-violet-300 shadow-sm border border-violet-100 dark:border-violet-900/50">
              <Award size={20} className="animate-pulse" />
            </div>
            <div>
              <h4 className="text-xs font-black text-violet-900 dark:text-violet-200 uppercase tracking-wide">Rango del Proyecto</h4>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-bold mt-0.5 leading-tight">Miembro activo del ecosistema Alive Maranata 2026.</p>
            </div>
          </div>
        ...
        <div className="xl:col-span-8 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/60 dark:border-slate-800 shadow-md hover:shadow-xl transition-all duration-300 p-6 space-y-6">
      ...
      {notification.isOpen && (
        <div className="fixed bottom-5 right-5 z-[99999] max-w-sm w-full bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-violet-100 dark:border-violet-900/60 p-4 flex items-center justify-between gap-3.5 animate-slideUp overflow-hidden">
          ...
          <button 
            ...
            className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer transition-colors p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg"
          >
            <X size={14} />
          </button>
        </div>
